...
async def _run_prepare(prep: dict | None) -> dict:
    ...
    Failure semantics:
      ...
      - sidecars:       all started; PIDs returned. Cleanup is the caller's
        responsibility (always tear down via _stop_sidecars).
    """
    out: dict = {
        "install_results": [],
        "command_results": [],
    ...
    for agent in prep.get("install_agents") or []:
        try:
            ...
            r = await _sa_proxy("POST", f"/v1/agents/{agent}/install",
    ...
    for cmd in prep.get("commands") or []:
        try:
            r = await _sa_proxy("POST", "/v1/processes/run",
    ...
    for f in prep.get("files") or []:
        try:
            await _sa_proxy("PUT", "/v1/fs/file", json_body=f, timeout=30)
    ...
    for s in prep.get("sidecars") or []:
        try:
            r = await _sa_proxy("POST", "/v1/processes", json_body=s, timeout=30)
...
async def _stop_sidecars(sidecars: list[dict]) -> None:
    ...
    for sc in sidecars:
        ...
        if not sid:
            continue
        try:
            await _sa_proxy("POST", f"/v1/processes/{sid}/stop",
...
async def _fetch_artifacts(spec: dict | None) -> dict:
    ...
    for path in spec.get("fetch") or []:
        try:
            r = await _sa_proxy("GET", "/v1/fs/file",
...
def _prepare_failed_result(run_id: str, prepare_result: dict, cid: str) -> dict:
